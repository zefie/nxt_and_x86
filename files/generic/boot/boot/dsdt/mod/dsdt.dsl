...
DefinitionBlock ("dsdt.aml", "DSDT", 2, "INSYDE", "INSYDE", 0x00000003)
{
    ...
    Name (PECS, One)
    Name (FTBL, 0x04)
    OperationRegion (GNVS, SystemMemory, 0x7CC64A90, 0x0335)
                                    ...
                                    Else
                                    {
                                        If ((_T_1 == 0x04))
                                        {
                                            Return (Buffer (0x5C)
                                            {
                                                ...
                                                /* 0048 */  0x89, 0x89, 0x02, 0x00, 0x14, 0x00, 0x01, 0x00,  /* ........ */
        ...
        Device (BMBT)
        {
            ...
            Method (_BST, 0, NotSerialized)  // _BST: Battery Status
            {
                If ((^^BMDR.AVBL == One))
                {
                    ...
                    If (((MARU > 0x8C) && (MARU < 0x1C84)))
                    {
                        Index (BSTP, 0x02) = MARU /* \_SB_.I2C5.BMBT.MARU */
                    }
                    Else
                    {
                        If (((MARU >= Zero) && (MARU < 0x8D)))
                        {
                            If (((^^BMDR.VOLT < 0x0EA6) && (^^BMDR.VOLT > 0x0BB8)))
                            {
                                Index (BSTP, 0x02) = MARU /* \_SB_.I2C5.BMBT.MARU */
                            }
                        }
